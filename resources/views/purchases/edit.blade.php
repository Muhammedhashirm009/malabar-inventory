...
@section('content')
<div class="mb-3"><a href="{{ route('purchases.show', $purchase) }}" class="btn btn-outline"><i data-lucide="arrow-left"></i> Back to Invoice</a></div>

<form method="POST" action="{{ route('purchases.update', $purchase) }}" x-data="purchaseForm({{ $purchase->items->map(fn($item) => [
...
])->toJson() }}, {{ json_encode(['id' => $purchase->supplier->id, 'name' => $purchase->supplier->name, 'phone' => $purchase->supplier->phone]) }})" @submit.prevent="submitForm($el)">
    @csrf
    @method('PUT')
    <div class="card mb-3" style="overflow: visible;">
        <div class="card-header"><h3>Purchase Details: {{ $purchase->invoice_number }}</h3></div>
        <div class="card-body">
            <div class="grid-3 gap-3">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="notes" class="form-control" value="{{ $purchase->notes }}" placeholder="Optional notes">
                </div>
            </div>
        </div>
    </div>
    ...
    <button type="submit" class="btn btn-primary btn-lg" :disabled="items.length === 0"><i data-lucide="save"></i> Update Purchase</button>
</form>
@endsection

@push('scripts')
<script>
function purchaseForm(initialItems = [], initialSupplier = null) {
    return {
        items: initialItems,
        searchQuery: '',
        searchResults: [],
        ...
        supplierQuery: initialSupplier ? initialSupplier.name : '',
        supplierResults: [],
