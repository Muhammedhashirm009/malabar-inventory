...
@section('content')
<div class="mb-3"><a href="{{ route('sales.show', $sale) }}" class="btn btn-outline"><i data-lucide="arrow-left"></i> Back to Invoice</a></div>

<form method="POST" action="{{ route('sales.update', $sale) }}" x-data="saleForm(
    {{ json_encode([
        'id' => $sale->customer->id,
    ...
    {{ $sale->items->map(fn($item) => [
        ...
        'mrp' => $item->mrp,
        'original_sale_rate' => (float) $item->sale_rate + (float) $item->discount,
        'getting_rate' => (float) $item->getting_rate,
        'discount' => (float) $item->discount,
        'stock' => (float) ($item->product->inventory->quantity ?? 0) + (float) $item->quantity,
    ...
    <div class="card mb-3" style="overflow: visible;">
        <div class="card-header"><h3>Sale Details: {{ $sale->invoice_number }}</h3></div>
        <div class="card-body">
            <div class="grid-3 gap-3">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="notes" class="form-control" value="{{ $sale->notes }}" placeholder="Optional notes">
                </div>
            </div>
        </div>
    </div>
    ...
    <button type="submit" class="btn btn-primary btn-lg" :disabled="items.length === 0 || !selectedCustomer || hasErrors"><i data-lucide="save"></i> Update Sale</button>
</form>
@endsection

@push('scripts')
<script>
function saleForm(initialCustomer = null, initialItems = []) {
    return {
        items: initialItems, searchQuery: '', searchResults: [], showDropdown: false, searchTimeout: null,
        selectedCustomer: initialCustomer, customerQuery: initialCustomer ? initialCustomer.name : '', customerResults: [], showCustomerDropdown: false, customerTimeout: null,
        init() {
            // Load last discounts for editing items without overwriting them
            this.items.forEach(item => {
                this.fetchLastDiscount(item, false);
            });
        },
        searchProducts() {
        ...
        selectCustomer(c) {
            ...
            this.items.forEach(item => {
                this.fetchLastDiscount(item, true);
            });
        },
        addProduct(p) {
            ...
            this.fetchLastDiscount(newItem, true);
            this.searchQuery = ''; this.searchResults = []; this.showDropdown = false;
            this.$nextTick(() => lucide.createIcons());
        },
        removeItem(idx) { this.items.splice(idx, 1); },
        fetchLastDiscount(item, overwriteDiscount = true) {
